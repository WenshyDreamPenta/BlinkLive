 ...
 *     author : wangmingxing
 *     time   : 2018/3/9
 *     desc   : OPENGLES Render
 * </pre>
 */
public class GLESRender implements IRender {
    private final Object syncRender = new Object();
    private GLESRenderThread glesRenderThread;

    @Override
    public void create(SurfaceTexture visualSurfaceTexture, int pixelFormat, int pixelWidth, int pixelHeight,
    ...
    private static class GLESRenderThread extends Thread {
        ...
        public GLESRenderThread(SurfaceTexture surfaceTexture, int pixelWidth, int pixelHeight, int visualHeight,
                int visualWidth, int pixelFormat) {
            mPixelWidth = pixelWidth;
            mPixelHeight = pixelHeight;
            mSurfaceTexture = surfaceTexture;
            quit = false;
            mSurfaceTexture = surfaceTexture;
            mSize = mPixelHeight * mPixelWidth;
            mVisualWidth = visualWidth;
            mVisualHeight = visualHeight;
            yBuf = ByteBuffer.allocateDirect(mSize);
            uBuf = ByteBuffer.allocateDirect(mSize >> 2);
            vBuf = ByteBuffer.allocateDirect(mSize >> 2);
            yTemp = new byte[mSize];
            uTemp = new byte[mSize >> 2];
            vTemp = new byte[mSize >> 2];
            Arrays.fill(uTemp, (byte) 0X7F);
            Arrays.fill(vTemp, (byte) 0X7F);
            uBuf.position(0);
            uBuf.put(vTemp).position(0);
            vBuf.position(0);
        ...
        private void drawFrame() {
            ...
            synchronized (syncBuff) {
                ...
                GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, mPixelWidth, mPixelHeight, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, yBuf);
                GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
                GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, uTexture[0]);
                GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0,
                        mPixelWidth >> 1, mPixelHeight >> 1, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, uBuf);
                GLES20.glActiveTexture(GLES20.GL_TEXTURE2);
                GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, vTexture[0]);
                GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0,
                        mPixelWidth >> 1, mPixelHeight >> 1, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE, vBuf);
            }
        ...
        private void initGLES() {
            ...
            if (EGL10.EGL_NO_DISPLAY == mEGLDisplay) {
                throw new RuntimeException(
                        "GLESRender,eglGetDisplay,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
            int versions[] = new int[2];
            if (!mEgl.eglInitialize(mEGLDisplay, versions)) {
                throw new RuntimeException(
                        "GLESRender,eglInitialize,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
            int configsNum[] = new int[1];
            EGLConfig configs[] = new EGLConfig[1];
            int configSpec[] = new int[]{EGL10.EGL_RENDERABLE_TYPE, EGL14.EGL_OPENGL_ES2_BIT, EGL10.EGL_RED_SIZE, 8, EGL10.EGL_GREEN_SIZE, 8, EGL10.EGL_BLUE_SIZE, 8, EGL10.EGL_DEPTH_SIZE, 0, EGL10.EGL_STENCIL_SIZE, 0, EGL10.EGL_NONE};
            mEgl.eglChooseConfig(mEGLDisplay, configSpec, configs, 1, configsNum);
            if (configsNum[0] <= 0) {
                throw new RuntimeException(
                        "GLESRender,eglChooseConfig,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
            ...
            if (null == mEGLSurface || EGL10.EGL_NO_SURFACE == mEGLSurface) {
                throw new RuntimeException(
                        "GLESRender,eglCreateWindowSurface,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
            int contextSpec[] = new int[]{EGL14.EGL_CONTEXT_CLIENT_VERSION, 2, EGL14.EGL_NONE};
            ...
            if (EGL10.EGL_NO_CONTEXT == mEGLcontext) {
                throw new RuntimeException(
                        "GLESRender,eglCreateContext,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
            if (!mEgl.eglMakeCurrent(mEGLDisplay, mEGLSurface, mEGLSurface, mEGLcontext)) {
                throw new RuntimeException(
                        "GLESRender,eglMakeCurrent,failed:" + GLUtils.getEGLErrorString(mEgl.eglGetError()));
            }
        ...
        private void initTexture() {
            ...
            GLES20.glVertexAttribPointer(aPostionLocation, SHAPE_COORD_PER_VERTEX, GLES20.GL_FLOAT, false,
                    SHAPE_COORD_PER_VERTEX * 4, mSquareVerticesBuffer);
            GLES20.glEnableVertexAttribArray(aTextureCoordLocation);
            GLES20.glVertexAttribPointer(aTextureCoordLocation, TEXTURE_COORD_PER_VERTEX, GLES20.GL_FLOAT, false,
